...
def generate_story():
    ...
    try:
        ...
        app.logger.info(f"Received prompt: {prompt[:50]}...")  # Log first 50 characters of prompt
        ...
        app.logger.info(f"Split scene into {total_paragraphs} paragraphs")

        result = []
        # Process each paragraph
        for index, paragraph in enumerate(paragraphs):
            if paragraph.strip():  # Ignore empty paragraphs
                app.logger.info(f"Processing paragraph {index + 1} of {total_paragraphs}. First few words: {' '.join(paragraph.split()[:5])}...")
                ...
                if image_url:
                    ...
                    app.logger.info(f"Failed to generate image for paragraph {index + 1}")
                ...
                if audio_url:
                    ...
                    app.logger.info(f"Failed to generate audio for paragraph {index + 1}")
        ...
        app.logger.info(f"Story generation process complete. Processed {total_paragraphs} paragraphs.")
